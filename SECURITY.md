...
behind `--engine=cli`, the default on terragrunt v1.x installs). When the
library engine is retired, `go-git`/`go-billy` drop out of the build entirely,
and the remaining `ignore` entries here can be removed.
