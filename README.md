...
- The project's autoplan `when_modified` covers the stack directory and the *local* `source`
  directories of its `unit` (and nested `stack`) blocks, e.g. a shared `units/` catalog. Remote
  sources cannot be watched.
- Units whose `path` points at a directory that already contains a `terragrunt.hcl` in the repo are
  treated as stack members and do **not** get individual projects.
- Catalog/source directories still get regular projects, matching the behavior you would get
  without `--enable-stacks`.
